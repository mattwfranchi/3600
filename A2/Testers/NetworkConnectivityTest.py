    ...
    def __init__(self, CRCServerModule, CRCClientModule, catch_exceptions):
        ...
        class NewCRCServerModule(CRCServerModule):
            ...
            def handle_messages(self, select_key, recv_data):
                print("[" + self.servername + "] \tReceived " + recv_data.decode())
                self.recvd_messages_asdqw.append(recv_data.decode())
            

            def write_data(self, server_name, message):
                print("[" + self.servername + "] \tSending " + message + " to " + server_name)
            ...
            def connect_to_server(self):
                ...
                if new_key:
                    ...
                    print("[" + self.servername + "] \tConnected to another server")
            ...
            def accept_new_connection(self, sock):
                ...
                if new_key:
                    self.special_map[self.connect_to_host] = new_key[0]
                    print("[" + self.servername + "] \tAccepted a connection from another server")
